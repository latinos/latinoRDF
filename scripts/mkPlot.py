...
#
# Core from:
#   https://github.com/latinos/LatinoAnalysis/blob/master/ShapeAnalysis/python/PlotFactory.py
#   https://github.com/latinos/LatinoAnalysis/blob/master/ShapeAnalysis/scripts/mkPlot.py
#
...
class PlotFactory:
    ...
    def __init__(self):
        ...
        self._plotNormalizedIncludeData          =  None
        self._plotNormalizedDistributions        =  None
        self._plotNormalizedDistributionsTHstack =  None
        self._showIntegralLegend                 =  0
        self._showRelativeRatio                  =  False
        self._showDataMinusBkgOnly               =  False
        self._removeWeight                       =  False
        self._invertXY                           =  False
        self._postFit                            =  None
        self._skipMissingNuisance                =  False
        self._removeMCStat                       =  False
        self._extraLegend                        =  None
        self._plotFancy                          =  False
        self._NoPreliminary                      =  False
    ...
    # _____________________________________________________________________________
    def setValues(self, inputFile, outputDirPlots, variables, cuts, samples, nuisances, lumi, plot, groupPlot, legend):
    ...
    def setConditions(self,
        ...
        self._scaleToPlot                         = scaleToPlot
        self._minLogC                             = minLogC
        self._maxLogC                             = maxLogC
        self._minLogCratio                        = minLogCratio
        self._maxLogCratio                        = maxLogCratio
        self._maxLinearScale                      = maxLinearScale
        self._linearOnly                          = linearOnly
        self._logOnly                             = logOnly
        self._plotNormalizedIncludeData           = plotNormalizedIncludeData
        self._plotNormalizedDistributions         = plotNormalizedDistributions
        self._plotNormalizedDistributionsTHstack  = plotNormalizedDistributionsTHstack
        self._showIntegralLegend                  = showIntegralLegend
        self._showRelativeRatio                   = showRelativeRatio
        self._showDataMinusBkgOnly                = showDataMinusBkgOnly
        self._removeWeight                        = removeWeight
        self._invertXY                            = invertXY
        self._postFit                             = postFit
        self._skipMissingNuisance                 = skipMissingNuisance
        self._removeMCStat                        = removeMCStat
        self._extraLegend                         = extraLegend
        self._plotFancy                           = plotFancy
        self._NoPreliminary                       = NoPreliminary
        self._removeAllMC                         = RemoveAllMC
        self._plotLinear                          = self._linearOnly or not self._logOnly
        self._plotLog                             = self._logOnly or not self._linearOnly
        self._minLogCdifference                   = self._minLogCratio
        self._maxLogCdifference                   = self._maxLogCratio


        print ("       self._silentMode                         = ",       self._silentMode                            )
        print ("       self._scaleToPlot                        = ",       self._scaleToPlot                           )
        print ("       self._minLogC                            = ",       self._minLogC                               )
        print ("       self._maxLogC                            = ",       self._maxLogC                               )
        print ("       self._minLogCratio                       = ",       self._minLogCratio                          )
        print ("       self._maxLogCratio                       = ",       self._maxLogCratio                          )
        print ("       self._maxLinearScale                     = ",       self._maxLinearScale                        )
        print ("       self._linearOnly                         = ",       self._linearOnly                            )
        print ("       self._logOnly                            = ",       self._logOnly                               )
        print ("       self._plotNormalizedIncludeData          = ",       self._plotNormalizedIncludeData             )
        print ("       self._plotNormalizedDistributions        = ",       self._plotNormalizedDistributions           )
        print ("       self._plotNormalizedDistributionsTHstack = ",       self._plotNormalizedDistributionsTHstack    )
        print ("       self._showIntegralLegend                 = ",       self._showIntegralLegend                    )
        print ("       self._showRelativeRatio                  = ",       self._showRelativeRatio                     )
        print ("       self._showDataMinusBkgOnly               = ",       self._showDataMinusBkgOnly                  )
        print ("       self._removeWeight                       = ",       self._removeWeight                          )
        print ("       self._invertXY                           = ",       self._invertXY                              )
        print ("       self._postFit                            = ",       self._postFit                               )
        print ("       self._skipMissingNuisance                = ",       self._skipMissingNuisance                   )
        print ("       self._removeMCStat                       = ",       self._removeMCStat                          )
        print ("       self._extraLegend                        = ",       self._extraLegend                           )
        print ("       self._plotFancy                          = ",       self._plotFancy                             )
        print ("       self._NoPreliminary                      = ",       self._NoPreliminary                         )
        print ("       self._removeAllMC                        = ",       self._removeAllMC                           )
        print ("       self._plotLinear                         = ",       self._plotLinear                            )
        print ("       self._plotLog                            = ",       self._plotLog                               )
        print ("       self._minLogCdifference                  = ",       self._minLogCdifference                     )
        print ("       self._maxLogCdifference                  = ",       self._maxLogCdifference                     )
    ...
    def makePlot(self):
        ...
        # prepare index.php
        #
        # it assumes the file index.php exists in the folder "plotsConfigurationsRDF" (as it should)
        # and you are working from a folder plotsConfigurationsRDF/<my analysis>/
        # or  you are working from a folder plotsConfigurationsRDF/<my analysis>/<sub folder>   --> this is the case for several years analyses
        # if more nested ... well, copy the index.php yourself ...
        #
        ...
        for cutName in self._cuts :
          ...
          for variableName, variable in self._variables.items():
              ...
              # if plotdef['isData'] == 1:
                ...
                for nuisanceName, nuisance in mynuisances.items():
                  ...
                  if 'name' in nuisance:
                    ...
                    shapeNameVars = tuple(cutName+"/"+variableName+'/histo_' + sampleName+"_"+nuisance['name']+var for var in ['up', 'do'])
                  else:
                    # shapeNameVars = tuple(cutName+"/"+variableName+'/histo_' + sampleName+"_"+nuisanceName+var for var in ['Up', 'Down'])
                    shapeNameVars = tuple(cutName+"/"+variableName+'/histo_' + sampleName+"_"+nuisanceName+var for var in ['up', 'do'])
                  ...
                  if 'type' in nuisance and nuisance['type'] == 'lnN':
                    ...
                    for ivar, shapeNameVar in enumerate(shapeNameVars):
                      ...
                      if histoVar :
                        ...
                        print (" This is bad, the nuisance ", nuisanceName, "[histo", shapeNameVar, "] is missing! You need to add it, maybe some jobs crashed?")
                        nuisanceHistos[ivar][nuisanceName] = histoVar
                      else :
                ...
                for ivar, nuisances_vy in enumerate([nuisances_vy_up, nuisances_vy_do]):
                  for nuisanceName, nuisance in mynuisances.items():
                    try:
                      ...
                      if 'scale' in plotdef:
                        # print ("histoVar = ", type(histoVar))
                        # histoVar.__class__ = ROOT.TH1D   # FIXME
                        # print (" --> histoVar = ", type(histoVar))
                    ...
                    try:
                      ...
                      vy = nuisances_vy[nuisanceName] = np.zeros_like(np.asarray(histo)[1:-1]) # FIXME check if ok like this ...

                    # get the background sum
                    if plotdef['isSignal'] == 0:   # ---> add the signal too????? See ~ 20 lines below
                      vy += np.asarray(histoVar)[1:-1] # FIXME check if ok like this ...
            ...
            if thsBackground.GetNhists() != 0:
              ...
              tgrMC_vy = np.asarray(last)[1:-1]  # , copy=True) #  [1:-1] # FIXME check if ok like this ...
              # print (" --------> lenght of tgrMC_vy = ", len(tgrMC_vy))
              ...
              nuisances_err2_up = np.asarray(last.GetSumw2())[1:-1] #  FIXME check if ok like this ...
              nuisances_err2_do = np.asarray(last.GetSumw2())[1:-1] #  FIXME check if ok like this ...
            ...
            # print (" --------> lenght of tgrData_vx = ", len(tgrData_vx))
            for iBin in range(0, len(tgrData_vx)) :
            ...
            if (not self._removeMCStat) or len(mynuisances.keys()) != 0 or histo_total:
              ...
              for iBin in range(0, len(tgrMC_vx)) :
                ...
                if histo_total:
                  ...
                  #
                  # print (" ::::                           DEBUG: iBin, tgrMC_vx[iBin], histo_total.GetBinContent(iBin+1) = ", iBin , ", ", tgrMC_vx[iBin], ", ", histo_total.GetBinContent(iBin+1))
                  ...
                  # print (" ::::                           DEBUG: iBin, tgrMC_evx[iBin], tgrMC_vx[iBin], tgrMC_vy[iBin], nuisances_err_do[iBin], nuisances_err_up[iBin] = ", iBin, ", ", tgrMC_evx[iBin], ", ", tgrMC_vx[iBin], ", ", tgrMC_vy[iBin], ", ", nuisances_err_do[iBin], ", ", nuisances_err_up[iBin])
	    ...
	    #     - then the superimposed MC
            ...
            if not self._removeAllMC :
              ...
              if len(mynuisances.keys()) != 0 or histo_total:
                  if self._showIntegralLegend == 0 :
...
if __name__ == '__main__':
    ...
    if os.path.exists(opt.plotFile) :
      ...
      handle.close()
      # clean the dictionary to remove globals due to "exec" funcionality
      plot      = {k: v for k, v in plot.items()      if not (k.startswith('__') and k.endswith('__'))}
      groupPlot = {k: v for k, v in groupPlot.items() if not (k.startswith('__') and k.endswith('__'))}
      legend    = {k: v for k, v in legend.items()    if not (k.startswith('__') and k.endswith('__'))}
    ...
    factory = PlotFactory()
    factory.setValues(inputFile, opt.outputDirPlots, variables, cuts, samples, nuisances, opt.lumi, plot, groupPlot, legend)
    ...
    factory.setConditions (
                           ...
                           opt.linearOnly,
                           opt.logOnly,
                           opt.plotNormalizedIncludeData,
                           opt.plotNormalizedDistributions,
                           opt.plotNormalizedDistributionsTHstack,
                           opt.showIntegralLegend,
                           opt.showRelativeRatio,
                           opt.showDataMinusBkgOnly,
                           opt.removeWeight,
                           opt.invertXY,
                           opt.postFit,
                           opt.skipMissingNuisance,
                           opt.removeMCStat,
                           opt.extraLegend,
                           opt.plotFancy,
                           opt.NoPreliminary,
